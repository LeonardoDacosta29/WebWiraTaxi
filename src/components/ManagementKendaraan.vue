    ...
    <div class="table-container">
      <div class="table-scrollable">
        <table class="table table-striped">
          ...
          <tbody>
            <tr v-for="vehicle in kendaraan" :key="vehicle.kendaraan_id">
              <td>{{ vehicle.merk }}</td>
...
export default {
  data() {
    return {
      kendaraan: [], // Ganti dari vehicles menjadi kendaraan
      drivers: [],
      isEditing: false,
      form: {
        merk: "",
  ...
  async mounted() {
    await this.fetchKendaraan(); // Ganti dari fetchVehicles menjadi fetchKendaraan
    await this.fetchDrivers(); // Pastikan fetchDrivers dipanggil di sini
  },
  methods: {
    async fetchKendaraan() {
      // Ganti dari fetchVehicles menjadi fetchKendaraan
      try {
        const response = await axios.get("/kendaraan");
  ...
  if (this.isEditing) {
    axios
      .put(`/kendaraan/${this.editingVehicleId}`, vehicle)
      .then(() => {
        this.fetchKendaraan(); 
    ...
    deleteVehicle(vehicleId) {
      if (confirm("Apakah Anda yakin ingin menghapus kendaraan ini?")) {
        axios
          .delete(`/kendaraan/${vehicleId}`)
          .then(() => {
            this.fetchKendaraan(); // Ganti dari fetchVehicles menjadi fetchKendaraan
